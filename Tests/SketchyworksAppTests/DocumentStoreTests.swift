...
final class DocumentStoreTests: XCTestCase {
    ...
    func testSavingToAPlainNameStillProducesAnSketchyworksDocument() throws {
        ...
        let final = typed.deletingLastPathComponent()
            .appendingPathComponent(SketchyworksFile.normalisedName(typed.lastPathComponent))
        ...
        let (back, _) = try SketchyworksFile.read(url: final)
...
final class RecentDocumentsTests: XCTestCase {
    ...
    private func scratch() -> UserDefaults {
        let suite = "sketchyworks.tests.\(UUID().uuidString)"
...
extension DocumentStoreTests {
    ...
    func testUnconfiguredBackendsSayWhichSettingToChange() {
        ...
        XCTAssertTrue(ModelConnector.Failure.notSignedIn.errorDescription?.contains("Sketchyworks") == true)
    }
}
...
extension DocumentStoreTests {
    ...
    /// perfect for Sketchyworks and for a vector program and useless to everything
    ...
    func testCopyingPutsABitmapOnThePasteboardForOtherApps() throws {
        ...
        XCTAssertNotNil(pb.data(forType: DocumentStore.pasteboardType),
                        "Sketchyworks's own format should still be there, and first")
