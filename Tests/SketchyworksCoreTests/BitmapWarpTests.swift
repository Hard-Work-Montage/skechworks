...
@Test func warpCornersSurviveTheFileFormat() throws {
    ...
    let data = try SketchyworksFile.write(document: doc, images: ["warp-file.png": whitePNG(width: 4, height: 4)])
    let (readDoc, _) = try SketchyworksFile.read(data)
...
@Test func lineHeightIsARatioAndOldFilesStillReadRight() throws {
    ...
    var data = try SketchyworksFile.write(document: doc, images: [:])
