...
//
// Saving Skechworks's model into a file still called .sketch makes something
...
@Test func someoneElsesFormatsAreImports() {
    for name in [ "Icons.sketch", "logo.svg", "board.fig", "art.ai", "old.eps", "spec.pdf" ] {
        #expect(SkechworksFile.isImportedFormat(URL(fileURLWithPath: "/tmp/\(name)")),
                "\(name) should force a Save As")
    }
}

@Test func ourOwnDocumentIsNotAnImport() {
    #expect(!SkechworksFile.isImportedFormat(URL(fileURLWithPath: "/tmp/Drawing.sw.png")))
}
...
@Test func aPlainPngIsNotDecidedHere() {
    ...
    #expect(!SkechworksFile.isImportedFormat(URL(fileURLWithPath: "/tmp/photo.png")))
}

@Test func theCaseOfTheExtensionDoesNotMatter() {
    #expect(SkechworksFile.isImportedFormat(URL(fileURLWithPath: "/tmp/Icons.SKETCH")))
    #expect(!SkechworksFile.isImportedFormat(URL(fileURLWithPath: "/tmp/Drawing.SW.PNG")))
}
