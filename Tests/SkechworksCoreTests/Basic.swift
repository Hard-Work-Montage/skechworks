...
@Test func textAlignmentSurvivesTheDocumentFormat() throws {
    ...
    let (back, _) = try SkechworksFile.read(try SkechworksFile.write(document: doc, images: [:]))
...
@Test func strippedPayloadIsReportedNotGuessed() {
    ...
    #expect(throws: (any Error).self) { try SkechworksFile.read(png) }
}
...
@Test func aSketchArchiveIsRefusedNotSwallowed() throws {
    ...
    // as a Skechworks document "succeeded" with a hollow 1-page/0-layer document,
    ...
    #expect(throws: SkechworksFile.ReadError.self) { try SkechworksFile.read(zip) }

    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("fake-\(UUID().uuidString).sketch")
    try zip.write(to: url)
    defer { try? FileManager.default.removeItem(at: url) }
    #expect(throws: SkechworksFile.ReadError.self) { _ = try DocumentSource.sw(url: url) }
}
...
@Test func clipboardRoundTripsLayersAndTheirImages() throws {
    ...
    let encoded = try SkechworksFile.encodeClipboard(layers: [group], images: ["images/abc.png": bytes])
    let back = try #require(SkechworksFile.decodeClipboard(encoded))
...
@Test func arcSurvivesSaveAndReload() throws {
    ...
    let data = try SkechworksFile.write(document: doc, images: [:])
    let (back, _) = try SkechworksFile.read(data)
...
@Test func pointTypesSurviveSavingAndReopening() throws {
    ...
    let (back, _) = try SkechworksFile.read(SkechworksFile.write(document: doc, images: [:]))
...
@Test func theRatioLockSurvivesSavingAndRegistersAsAnEdit() throws {
    ...
    var doc = Document()
    doc.pages = [page]
    let data = try SkechworksFile.write(document: doc, images: ["p.png": Data([1])])
    let (back, _) = try SkechworksFile.read(data)
...
@Test func aRenamedDocumentStillHoldsEveryLayer() throws {
    ...
    let written = try SkechworksFile.write(document: doc, images: [:])
    ...
    let (back, _) = try SkechworksFile.read(url: renamed)
...
@Test func bitmapAdjustmentsAndCropSurviveTheFormat() throws {
    ...
    var doc = Document()
    doc.pages = [page]
    let data = try SkechworksFile.write(document: doc, images: ["photo.png": Data([0x89, 0x50])])
    let back = try SkechworksFile.read(data).document.pages[0].layers[0]
...
@Test func aDeletedImageDoesNotRideAlongInTheArchive() throws {
    ...
    let back = try Zip.read(try SkechworksFile.write(document: doc, images: images))
