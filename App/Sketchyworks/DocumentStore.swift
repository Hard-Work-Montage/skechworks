...
final class DocumentStore: ObservableObject {
    ...
    private func needsAccount(_ tool: String) {
        let said = "\(tool) needs your Sketchyworks account. Connect it in Settings."
    ...
    func open(_ url: URL) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            if ext == "svg" {
                ...
                do {
                    ...
                    if ext == "sketch" {
                        ...
                        l.name = SketchyworksFile.baseName(url.lastPathComponent)
            ...
            await MainActor.run { [made, failure, notes, importedImage] in
                ...
                // Anything that wasn't already a Sketchyworks document has no
                // .sw.png behind it, so saving must ask where rather than
                // write back over what was opened.
                //
                // A .sketch especially. Saving Sketchyworks's model into a file
                ...
                if SketchyworksFile.isImportedFormat(url) || importedImage {
                    self.url = nil
                    self.isDirty = true
                }
                self.pageIndex = 0
    ...
    func vectorizeSelection(style: String = "color") {
        ...
        guard !(Credentials.get(.sketchyworksToken) ?? "").isEmpty else {
    ...
    static let pasteboardType = NSPasteboard.PasteboardType("com.sketchyworks.layers")
    ...
    func copySelection() {
        ...
        //
        // Sketchyworks takes the native one and gets everything back. A vector
        ...
        if let d = try? SketchyworksFile.encodeClipboard(layers: layers, images: images) {
    ...
    func paste(at targetOrigin: CGPoint? = nil, above anchor: String? = nil) {
        ...
        guard let src = source, var p = page,
              ...
              let (layers, assets) = SketchyworksFile.decodeClipboard(data), !layers.isEmpty else {
            pasteExternal()
            return
        }
    ...
    /// Pasting something that didn't come from Sketchyworks: an image copied out of
    ...
    private func apply(_ p: Page, at idx: Int, src: DocumentSource) {
        ...
        if !broken.isEmpty {
            ...
            Logger(subsystem: "com.sketchyworks.Sketchyworks", category: "geometry")
    ...
    static var recoveryDir: URL {
        recoveryDirOverride ?? FileManager.default
            ...
            .appendingPathComponent("Sketchyworks/Recovery", isDirectory: true)
    }
    ...
    private static let recoveryQueue = DispatchQueue(label: "com.sketchyworks.autosave", qos: .utility)
    ...
    static func pendingRecoveries() -> [Recovery] {
        ...
        return entries.filter { SketchyworksFile.isOwnDocumentName($0.lastPathComponent) }.map { snap in
            let sidecar = recoveryDir.appendingPathComponent(
                SketchyworksFile.baseName(snap.lastPathComponent) + ".json")
            var original: URL?
    ...
    func restoreFromRecovery(_ rec: Recovery) {
        ...
        guard let (doc, images) = try? SketchyworksFile.read(url: rec.snapshot) else { return }
    ...
    func saveAs(completion: ((Bool) -> Void)? = nil) {
        ...
        panel.nameFieldStringValue = SketchyworksFile.baseName(url?.lastPathComponent ?? "Untitled")
        ...
        panel.message = "Saved as a Sketchyworks document (.sw.png)"
        ...
        out = out.deletingLastPathComponent()
            .appendingPathComponent(SketchyworksFile.normalisedName(out.lastPathComponent))
        url = out
        writeToDisk(completion: completion)
    }
    ...
    func openPanel() {
        ...
        p.message = "Open a Sketchyworks document (.sw.png) or a Sketch file"
