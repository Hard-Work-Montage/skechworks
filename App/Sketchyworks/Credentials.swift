...
enum Credentials {
    ...
    enum Slot: String {
        ...
        case sketchyworksToken = "sketchyworks.token"
    }

    private static let service = "com.sketchyworks.Sketchyworks"

    /// Where the same items lived while the app was called Accomplice. A rename
    /// must not sign anyone out, so a miss under the new name reads the old
    /// one and carries it across.
    private static let legacyService = "com.accomplice.Accomplice"
    private static func legacyAccount(_ slot: Slot) -> String {
        slot == .sketchyworksToken ? "accomplice.token" : slot.rawValue
    }

    static func get(_ slot: Slot) -> String? {
        if let s = read(service: service, account: slot.rawValue) { return s }
        guard let old = read(service: legacyService, account: legacyAccount(slot)) else { return nil }
        set(slot, old)
        return old
    }
    ...
    private static let log = Logger(subsystem: "com.sketchyworks.Sketchyworks", category: "credentials")
