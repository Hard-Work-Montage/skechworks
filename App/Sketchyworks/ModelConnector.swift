...
/// Talks to Sketchyworks's own service, or to OpenRouter with the user's key.
///
...
struct ModelConnector {
    ...
    enum Backend: String, CaseIterable, Identifiable, Sendable {
        case openRouter, sketchyworks
        var id: String { rawValue }

        var title: String {
            switch self {
            case .openRouter: return "OpenRouter account"
            case .sketchyworks: return "Sketchyworks account"
            }
        }
        ...
        var detail: String {
            ...
            case .sketchyworks:
                return "Sign in and use Sketchyworks tokens. No download, no separate account."
            }
        }
    }

    struct Settings {
        var backend: Backend = .sketchyworks
        var openRouterModel = "anthropic/claude-sonnet-4.5"
        /// Where Sketchyworks accounts live. Was configurable while sketchy.works
        ...
        let sketchyworksHost = "https://sketchy.works"
        ...
        var sketchyworksToken: String { Credentials.get(.sketchyworksToken) ?? "" }
    ...
    enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notSignedIn: return "Sign in to Sketchyworks in Settings, or switch to another option."
    ...
    private func complete(messages: [[String: Any]], purpose: Purpose) async throws -> (String, String?) {
        ...
        case .sketchyworks: return try await sketchyworks(messages, purpose: purpose)
        }
    }

    /// Sketchyworks's own endpoint, which fronts a provider and bills tokens to the
    ...
    private func sketchyworks(_ messages: [[String: Any]], purpose: Purpose = .chat) async throws -> (String, String?) {
        let token = settings.sketchyworksToken
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: settings.sketchyworksHost + "/api/v1/chat/completions") else {
    ...
    private func openRouter(_ messages: [[String: Any]]) async throws -> String {
        ...
        let json = try await post(url, body: body, headers: [
            ...
            "HTTP-Referer": "https://sketchy.works",
            "X-Title": "Sketchyworks",
        ])
        guard let choices = json["choices"] as? [[String: Any]],
              let message = choices.first?["message"] as? [String: Any],
    ...
    static func accountBalance() async throws -> (email: String, credits: Double) {
        let token = Credentials.get(.sketchyworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().sketchyworksHost + "/api/v1/me") else {
            throw Failure.unreachable("bad url")
        }
    ...
    static var creditsURL: URL { URL(string: Settings().sketchyworksHost + "/credits")! }
    ...
    static func vectorize(png: Data, style: String = "color") async throws -> (svg: String, remaining: Double?) {
        let token = Credentials.get(.sketchyworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().sketchyworksHost + "/api/v1/vectorize") else {
            throw Failure.unreachable("bad url")
        }
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
    ...
    static func remove(png: Data, rect: CGRect, mode: String = "remove") async throws -> (png: Data, remaining: Double?) {
        let token = Credentials.get(.sketchyworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().sketchyworksHost + "/api/v1/remove") else {
            throw Failure.unreachable("bad url")
        }
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
