...
/// Talks to Skechworks's own service, or to OpenRouter with the user's key.
///
...
struct ModelConnector {
    ...
    enum Backend: String, CaseIterable, Identifiable, Sendable {
        case openRouter, skechworks
        var id: String { rawValue }

        var title: String {
            switch self {
            case .openRouter: return "OpenRouter account"
            case .skechworks: return "Skechworks account"
            }
        }
        ...
        var detail: String {
            ...
            case .skechworks:
                return "Sign in and use Skechworks tokens. No download, no separate account."
            }
        }
    }

    struct Settings {
        var backend: Backend = .skechworks
        var openRouterModel = "anthropic/claude-sonnet-4.5"
        /// Where Skechworks accounts live. Was configurable while skechworks.com
        ...
        let skechworksHost = "https://skechworks.com"
        ...
        var skechworksToken: String { Credentials.get(.skechworksToken) ?? "" }
    ...
    enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notSignedIn: return "Sign in to Skechworks in Settings, or switch to another option."
    ...
    private func complete(messages: [[String: Any]], purpose: Purpose) async throws -> (String, String?) {
        ...
        case .skechworks: return try await skechworks(messages, purpose: purpose)
        }
    }

    /// Skechworks's own endpoint, which fronts a provider and bills tokens to the
    ...
    private func skechworks(_ messages: [[String: Any]], purpose: Purpose = .chat) async throws -> (String, String?) {
        let token = settings.skechworksToken
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: settings.skechworksHost + "/api/v1/chat/completions") else {
    ...
    private func openRouter(_ messages: [[String: Any]]) async throws -> String {
        ...
        let json = try await post(url, body: body, headers: [
            ...
            "HTTP-Referer": "https://skechworks.com",
            "X-Title": "Skechworks",
        ])
        guard let choices = json["choices"] as? [[String: Any]],
              let message = choices.first?["message"] as? [String: Any],
    ...
    static func accountBalance() async throws -> (email: String, credits: Double) {
        let token = Credentials.get(.skechworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().skechworksHost + "/api/v1/me") else {
            throw Failure.unreachable("bad url")
        }
    ...
    static var creditsURL: URL { URL(string: Settings().skechworksHost + "/credits")! }
    ...
    static func vectorize(png: Data, style: String = "color") async throws -> (svg: String, remaining: Double?) {
        let token = Credentials.get(.skechworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().skechworksHost + "/api/v1/vectorize") else {
            throw Failure.unreachable("bad url")
        }
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
    ...
    static func remove(png: Data, rect: CGRect, mode: String = "remove") async throws -> (png: Data, remaining: Double?) {
        let token = Credentials.get(.skechworksToken) ?? ""
        guard !token.isEmpty else { throw Failure.notSignedIn }
        guard let url = URL(string: Settings().skechworksHost + "/api/v1/remove") else {
            throw Failure.unreachable("bad url")
        }
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
