...
private struct ColorPickerPane: View {
    ...
    init(color: SkechworksCore.Color, supportsOpacity: Bool,
         onChange: @escaping (SkechworksCore.Color) -> Void) {
    ...
    private var current: SkechworksCore.Color {
        let ns = NSColor(hue: hue, saturation: sat, brightness: bri, alpha: alpha)
            .usingColorSpace(.sRGB) ?? .black
        return SkechworksCore.Color(r: ns.redComponent, g: ns.greenComponent,
    ...
    private func commitHex() {
        guard let c = SkechworksCore.Color(hex: hexTyped, alpha: alpha) else {
    ...
    private static func digits(_ c: SkechworksCore.Color) -> String {
