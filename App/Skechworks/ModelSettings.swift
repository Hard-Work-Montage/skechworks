...
struct ModelSettings: View {
    @AppStorage("ai.backend") private var backend = ModelConnector.Backend.skechworks.rawValue
    ...
    private var chosen: ModelConnector.Backend {
        ModelConnector.Backend(rawValue: backend) ?? .skechworks
    }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("Skechworks account")
            } footer: {
                Text("Everything else in Skechworks is free. Credits pay for the "
    ...
    private var openRouterSection: some View {
        Group {
            ...
            Text("Signing in creates a key scoped to Skechworks — we never see your "
    ...
    private var skechworksSection: some View {
        Group {
            account(connected: Credentials.has(.skechworksToken),
                    connectedLabel: "Signed in to Skechworks",
                    connectLabel: "Sign in to Skechworks…",
                    connect: { try await flow.connect(.skechworks(base: ModelConnector.Settings().skechworksHost)) },
                    disconnect: { Credentials.set(.skechworksToken, nil) })
        ...
        HStack {
            if connected {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if connectedLabel.contains("Skechworks") {
                        if expired {
                ...
                .task(id: "\(credentialsRevision)-\(balanceRevision)") {
                    guard connectedLabel.contains("Skechworks") else { return }
