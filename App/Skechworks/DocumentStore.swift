...
final class DocumentStore: ObservableObject {
    ...
    private func needsAccount(_ tool: String) {
        let said = "\(tool) needs your Skechworks account. Connect it in Settings."
    ...
    func open(_ url: URL) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            if ext == "svg" {
                ...
                do {
                    ...
                    if ext == "sketch" {
                        ...
                        l.name = SkechworksFile.baseName(url.lastPathComponent)
            ...
            await MainActor.run { [made, failure, notes, importedImage] in
                ...
                if SkechworksFile.isImportedFormat(url) || importedImage {
                    self.url = nil
                    self.isDirty = true
                }
                self.pageIndex = 0
    ...
    func vectorizeSelection(style: String = "color") {
        ...
        chat.note(entry, style == "lineart" ? "Black and white" : "Full color")
    ...
    func paste(at targetOrigin: CGPoint? = nil, above anchor: String? = nil) {
        ...
        guard let src = source, var p = page,
              ...
              let (layers, assets) = SkechworksFile.decodeClipboard(data), !layers.isEmpty else {
            pasteExternal()
            return
        }
    ...
    /// Pasting something that didn't come from Skechworks: an image copied out of
    ...
    private func apply(_ p: Page, at idx: Int, src: DocumentSource) {
        ...
        if !broken.isEmpty {
            ...
            Logger(subsystem: "com.skechworks.Skechworks", category: "geometry")
    ...
    static var recoveryDir: URL {
        recoveryDirOverride ?? FileManager.default
            ...
            .appendingPathComponent("Skechworks/Recovery", isDirectory: true)
    }
    ...
    private static let recoveryQueue = DispatchQueue(label: "com.skechworks.autosave", qos: .utility)
    ...
    static func pendingRecoveries() -> [Recovery] {
        ...
        return entries.filter { SkechworksFile.isOwnDocumentName($0.lastPathComponent) }.map { snap in
    ...
    func restoreFromRecovery(_ rec: Recovery) {
        ...
        guard let (doc, images) = try? SkechworksFile.read(url: rec.snapshot) else { return }
        ...
        if SkechworksFile.isOwnDocumentName(url.lastPathComponent),
           !url.lastPathComponent.lowercased().hasSuffix("." + SkechworksFile.suffix) {
            ...
            url = url.deletingLastPathComponent()
                .appendingPathComponent(SkechworksFile.normalisedName(url.lastPathComponent))
            self.url = url
        }
        let out = url
        let retire = legacy
