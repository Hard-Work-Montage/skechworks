...
enum ChatArchive {
    ...
    static var directory: URL {
        directoryOverride ?? FileManager.default
            ...
            .appendingPathComponent("Skechworks/Conversations", isDirectory: true)
    }
    ...
    static func load(for url: URL) -> [ChatMessage] {
        ...
        return stored.map { s in
            ...
            if s.running == true {
                m.role = .error
                m.text = "\(s.text) stopped when Skechworks closed."
            }
            return m
        }
    }
    ...
    static func save(_ messages: [ChatMessage], for url: URL) {
        ...
        do {
            ...
            Logger(subsystem: "com.skechworks.Skechworks", category: "chat")
