...
/// A local MCP endpoint, so Claude Code and friends can drive Skechworks directly.
///
...
final class MCPServer {
    ...
    private func handle(request: Data) -> Data {
        ...
        case "initialize":
            return rpc(id, [
                ...
                "serverInfo": ["name": "skechworks", "version": "0.1"],
            ])
    ...
    private func call(_ name: String, _ args: [String: Any]) -> String {
        guard let store else { return "No document is open in Skechworks." }
        switch name {
