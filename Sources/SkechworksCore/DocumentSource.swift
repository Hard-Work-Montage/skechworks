...
/// goes through `lock`.
/// Which files are Skechworks documents rather than content to place.
///
...
public enum DocumentKind {
    public static func isDocument(_ url: URL) -> Bool {
        ...
        return SkechworksFile.isOwnDocumentName(n)
...
public final class DocumentSource: @unchecked Sendable {
    ...
    public static func sw(url: URL) throws -> DocumentSource {
        ...
        let z: [String: Data]
        do { z = try Zip.read(data) } catch { throw SkechworksFile.ReadError.stripped }
        guard let docData = z["document.json"],
            ...
            throw SkechworksFile.ReadError.stripped
        }
        ...
        if dj["_class"] != nil || entries.contains(where: { !($0["file"] is String) }) {
            throw SkechworksFile.ReadError.notSkechworks
        }
        ...
        return DocumentSource(sourceApp: dj["importedFrom"] as? String,
            ...
            return SkechworksFile.parsePage(d)
        }
    }
