...
public struct SketchyworksFile {
    ...
    public static func write(document: Document,
        ...
        let doc: [String: Any] = [
            ...
            "generator": "Sketchyworks liberator",
    ...
    public static func encodeClipboard(layers: [Layer], images: [String: Data]) throws -> Data {
        ...
        return try json(["format": "sw-clipboard",
    ...
    public static func decodeClipboard(_ data: Data) -> (layers: [Layer], images: [String: Data])? {
        guard let j = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              j["format"] as? String == "sw-clipboard",
    ...
    public enum ReadError: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .notSketchyworks:
                return "not a Sketchyworks document — the archive belongs to another app"
            }
        }
    }

    /// The extension every Sketchyworks document ends with.
    ...
    /// Whether this name is one of our documents, under either name.
    public static func isOwnDocumentName(_ name: String) -> Bool {
        let n = name.lowercased()
        return ownTails.contains { n.hasSuffix($0) }
    }

    /// The name without the compound extension: "Coin.sw.png" -> "Coin".
    ...
    public static func isImportedFormat(_ url: URL) -> Bool {
        ...
        // An .sw.png is ours even though it ends in .png, and so is an .acmplc.png.
        if isOwnDocumentName(name) { return false }
        return [ "sketch", "svg", "fig", "ai", "eps", "pdf" ]
            .contains(url.pathExtension.lowercased())
    }

    public static func baseName(_ name: String) -> String {
        let base = stripOwnExtension(name)
    ...
    private static func stripOwnExtension(_ name: String) -> String {
        ...
        while stripped {
            stripped = false
            for tail in [".png", ".sw", ".acmplc"] where base.lowercased().hasSuffix(tail) {
    ...
    /// but the compound extension is what makes the file open in Sketchyworks rather than
    /// Preview, so it's put back.
    public static func normalisedName(_ name: String) -> String {
        if name.lowercased().hasSuffix("." + suffix) {
            return name
        }
        // Strip whatever is there, old name included, then put the new one on:
        // saving an Accomplice-era file is how it becomes a Sketchyworks one.
        var base = stripOwnExtension(name)
    ...
    public static func read(_ data: Data) throws -> (document: Document, images: [String: Data]) {
        ...
        if dj["_class"] != nil { throw ReadError.notSketchyworks }
    ...
    private static func readme(_ d: Document, _ cover: Int) -> String {
        """
        This file is a Sketchyworks document (.sw.png).
        ...
        You do not need Sketchyworks to get your work back out of this file. That is
